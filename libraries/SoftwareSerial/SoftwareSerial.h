...
#define PWM_DIV              4 // appears to be 48MHz clock / 4, can't find the config
#define TIMER_ISR_OVERHEAD   15 // ticks, at CYDEV_BCLK__SYSCLK__HZ / 4 rate, determined experimentally
#define RX_ISR_OVERHEAD      36 // ticks, at CYDEV_BCLK__SYSCLK__HZ / 4 rate, determined experimentally
...
class SoftwareSerial : public Stream {
  private:
    ...
    uint32_t _debugBitMask;

    // Expressed as timer tick delays (must never be 0!)
...
public:
    // static data
    static uint8_t _serialState;
    static uint8_t _bit;
    static uint8_t _rbyte;
...
private:
  ...
  public:
    ...
    void setDebugPin(uint8_t debugPin);
    ...
    static inline void handle_pin_interrupt() __attribute__((__always_inline__));
    static inline void handle_timer_interrupt() __attribute__((__always_inline__));
};
